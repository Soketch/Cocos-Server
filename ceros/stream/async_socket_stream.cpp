...
namespace cero
{
    ...
    void AsyncSocketStream::Ctx::doRsp()
    {
        ...
        if (!cero::Atomic::compareAndSwapBool(scheduler, scd, (Scheduler *)nullptr))
        {
            return;
        }
        if (!scd || !fiber)
    ...
    bool AsyncSocketStream::start()
    {
        if (!m_iomanager)
        {
            m_iomanager = cero::IOManager::GetThis();
        }
        if (!m_worker)
        {
            m_worker = cero::IOManager::GetThis();
        }
    ...
    {
        ASSERT(m_iomanager == cero::IOManager::GetThis());
    ...
    AsyncSocketStream::ptr AsyncSocketStreamManager::get()
    {
        ...
        {
            auto idx = cero::Atomic::addFetch(m_idx, 1);
