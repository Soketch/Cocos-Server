...
namespace cero
{
    ...
    ServiceItemInfo::ptr ServiceItemInfo::Create(const std::string &ip_and_port, const std::string &data)
    {
        ...
        auto port = cero::TypeUtil::Atoi(ip_and_port.substr(pos + 1));
    ...
    {
        cero::RWMutex::WriteLock lock(m_mutex);
        m_queryInfos = v;
    }
    ...
    void IServiceDiscovery::listServer(std::unordered_map<std::string, std::unordered_map<std::string, std::unordered_map<uint64_t, ServiceItemInfo::ptr>>> &infos)
    {
        cero::RWMutex::ReadLock lock(m_mutex);
        infos = m_datas;
    }

    void IServiceDiscovery::listRegisterServer(std::unordered_map<std::string, std::unordered_map<std::string, std::unordered_map<std::string, std::string>>> &infos)
    {
        cero::RWMutex::ReadLock lock(m_mutex);
    ...
    void ZKServiceDiscovery::start()
    {
        ...
        m_client.reset(new cero::ZKClient);
        bool b = m_client->init(m_hosts, 6000, std::bind(&ZKServiceDiscovery::onWatch, self, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
        if (!b)
        {
            LOG_ERROR(g_logger) << "ZKClient init fail, hosts=" << m_hosts;
        }
        m_timer = cero::IOManager::GetThis()->addTimer(60 * 1000, [self, this]()
                                                       {
        m_isOnTimer = true;
        onZKConnect("", m_client);
        m_isOnTimer = false; }, true);
    }
    ...
    bool ZKServiceDiscovery::queryInfo(const std::string &domain, const std::string &service)
    {
        if (service != "all")
        {
            ...
            std::string new_val(1024, 0);
            int32_t rt = m_client->create(path + "/" + m_selfInfo, m_selfData, new_val, &ZOO_OPEN_ACL_UNSAFE, ZKClient::FlagsType::EPHEMERAL);
            if (rt == ZOK)
            {
                return true;
            }
        ...
        }
        {
            ...
            if (it->second.count(service) == 0 && it->second.count("all") == 0)
            {
                LOG_ERROR(g_logger) << "get_children path=" << path
                                    ...
                                    << cero::Join(it->second.begin(), it->second.end(), ",");
                return false;
            }
        }
