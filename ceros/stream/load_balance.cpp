...
#undef XX
    ...
    void LoadBalanceItem::close()
    {
        if (m_stream)
        {
            ...
            cero::WorkerMgr::GetInstance()->schedule("service_io", [stream]()
                                                     { stream->close(); });
        }
    }
    ...
    void LoadBalance::update(const std::unordered_map<uint64_t, LoadBalanceItem::ptr> &adds, std::unordered_map<uint64_t, LoadBalanceItem::ptr> &dels)
    {
        RWMutexType::WriteLock lock(m_mutex);
        for (auto &i : dels)
        {
    ...
    std::string LoadBalance::statusString(const std::string &prefix)
    {
        ...
        lock.unlock();
        std::stringstream ss;
        ss << prefix << "init_time: " << cero::Time2Str(m_lastInitTime / 1000) << std::endl;
        for (auto &i : datas)
        {
    ...
    float HolderStats::getWeight(float rate)
    {
        ...
        return std::min((m_oks * 1.0 / (m_usedTime + 1)) * 2.0, 50.0) * (1 - 4.0 * m_timeouts / base) * (1 - 1 * m_doing / base) * (1 - 10.0 * m_errs / base) * rate;
    ...
    void SDLoadBalance::onServiceChange(const std::string &domain, const std::string &service, const std::unordered_map<uint64_t, ServiceItemInfo::ptr> &old_value, const std::unordered_map<uint64_t, ServiceItemInfo::ptr> &new_value)
    {
    ...
    {
        m_sd->setServiceCallback(std::bind(&SDLoadBalance::onServiceChange, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
        m_sd->start();
    }
