...
namespace cero
{
    ...
    static cero::ConfigVar<uint64_t>::ptr g_tcp_server_read_timeout =
        cero::Config::Lookup("tcp_server.read_timeout", (uint64_t)(60 * 1000 * 2),
                             "tcp server read timeout");

    static cero::Logger::ptr g_logger = LOG_NAME("system");

    TcpServer::TcpServer(cero::IOManager *worker,
                         cero::IOManager *io_worker,
                         cero::IOManager *accept_worker)
        : m_worker(worker), m_ioWorker(io_worker), m_acceptWorker(accept_worker),
          m_recvTimeout(g_tcp_server_read_timeout->getValue()),
          m_name("cero/1.0.0"), m_isStop(true)
    {
    }
    ...
    bool TcpServer::bind(cero::Address::ptr addr, bool ssl)
    {
