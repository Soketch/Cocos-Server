...
namespace cero
{
    ...
    public:
        ...
        static WorkerGroup::ptr Create(uint32_t batch_size, cero::Scheduler *s = cero::Scheduler::GetThis())
        {
            return std::make_shared<WorkerGroup>(batch_size, s);
        }

        WorkerGroup(uint32_t batch_size, cero::Scheduler *s = cero::Scheduler::GetThis());
        ~WorkerGroup();
    ...
    public:
        ...
        void schedule(const std::string &name, FiberOrCb fc, int thread = -1)
        {
            ...
            {
                static cero::Logger::ptr s_logger = LOG_NAME("system");
    ...
    typedef cero::Singleton<WorkerManager> WorkerMgr;
