...
namespace cero
{
    ...
    static cero::ConfigVar<std::map<std::string, std::map<std::string, std::string>>>::ptr g_worker_config = cero::Config::Lookup("workers", std::map<std::string, std::map<std::string, std::string>>(), "worker config");

    WorkerGroup::WorkerGroup(uint32_t batch_size, cero::Scheduler *s)
    ...
    bool WorkerManager::init(const std::map<std::string, std::map<std::string, std::string>> &v)
    {
        for (auto &i : v)
        {
            ...
            int32_t thread_num = cero::GetParamValue(i.second, "thread_num", 1);
            int32_t worker_num = cero::GetParamValue(i.second, "worker_num", 1);
