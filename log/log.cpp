...
#define LOG_COLOR(level)                                                      \
    ((level) == LogLevel::DEBUG ? BLUE : (level) == LogLevel::INFO  ? WHITE   \
                                     : (level) == LogLevel::SUCCESS ? GREEN   \
...
#undef XX
    ...
    void Logger::setFormatter(const std::string &val)
    {
        ...
        cero::LogFormatter::ptr new_val(new cero::LogFormatter(val));
        if (new_val->isError())
        {
    ...
    void Logger::success(LogEvent::ptr event)
    {
        log(LogLevel::SUCCESS, event);
    }
...
#undef XX
    ...
    cero::ConfigVar<std::set<LogDefine>>::ptr g_log_defines =
        cero::Config::Lookup("logs", std::set<LogDefine>(), "logs config");
    ...
    struct LogIniter
    {
        LogIniter()
        {
            ...
            for(auto& i : new_value) {
                ...
                for(auto& a : i.appenders) {
                    cero::LogAppender::ptr ap;
                    if(a.type == 1) {
                        ap.reset(new FileLogAppender(a.file));
                    } else if(a.type == 2) {
                        if(!cero::EnvMgr::GetInstance()->has("d")) {
